...
@asset_check(
    asset=lab_renta_assets.raw_renta,
...
@asset_check(
    asset=lab_renta_assets.raw_nombres,
...
@asset_check(
    asset=lab_renta_assets.raw_estudios,
...
@asset_check(
    asset=lab_renta_assets.df_renta,
...
@asset_check(
    asset=lab_renta_assets.df_nombres,
...
@asset_check(
    asset=lab_renta_assets.df_estudios,
...
@asset_check(
    asset=lab_renta_assets.df_renta_isla,
...
@asset_check(
    asset=lab_renta_assets.df_renta_edu,
    description="Verifica que el cruce entre renta y educación conserve una cobertura suficiente de municipios con dato educativo disponible."
)
def check_df_renta_edu_cobertura_merge(df_renta_edu: pd.DataFrame) -> AssetCheckResult:
    municipios = df_renta_edu["Tipo"] == "Municipio"
    total = int(municipios.sum())
    con_dato = int(df_renta_edu.loc[municipios, "Educación superior (%)"].notna().sum())
    cobertura = 0 if total == 0 else round((con_dato / total) * 100, 2)
    ...
    return AssetCheckResult(
        passed=passed,
        metadata={
            "filas_municipales": MetadataValue.int(total),
            "filas_con_educacion": MetadataValue.int(con_dato),
            "cobertura_pct": MetadataValue.float(cobertura),
            "principio_gestalt": "Conectividad (El cruce debe conservar suficiente información compartida)",
            "mensaje": "El merge con educación debe cubrir la mayoría de municipios."
        }
    )
...
@asset_check(
    asset=lab_renta_assets.graph_01,
...
def check_graph_01_exportado() -> AssetCheckResult:
    path = VIS_DIR / "graph_01_distribucion_renta.png"
    existe = path.exists()
...
@asset_check(
    asset=lab_renta_assets.graph_02,
...
def check_graph_02_exportado() -> AssetCheckResult:
    path = VIS_DIR / "graph_02_linea_gomera_sueldos_salarios.png"
    existe = path.exists()
    tamano = path.stat().st_size if existe else 0
...
@asset_check(
    asset=lab_renta_assets.graph_03,
    description="Verifica que el heatmap de prestaciones por desempleo para La Palma se haya exportado correctamente."
)
def check_graph_03_exportado() -> AssetCheckResult:
    path = VIS_DIR / "graph_03_heatmap_palma_desempleo.png"
    existe = path.exists()
    tamano = path.stat().st_size if existe else 0
    ...
    return AssetCheckResult(
        ...
        metadata={
            "archivo": MetadataValue.path(str(path)),
            "tamano_bytes": tamano,
            "principio_gestalt": "Proximidad (la matriz visual debe poder generarse completa)",
...
@asset_check(
    asset=lab_renta_assets.graph_04,
...
def check_graph_04_exportado() -> AssetCheckResult:
    path = VIS_DIR / "graph_04_scatter_palma_salario_educación_2023.png"
    existe = path.exists()
    tamano = path.stat().st_size if existe else 0
    ...
    return AssetCheckResult(
        ...
        metadata={
            ...
            "mensaje": "El gráfico 4 debe guardarse correctamente."
        }
    )